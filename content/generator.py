...
# 이 버튼은 API 요금이 붙는 경로다. 요금을 쓰지 않기로 했으므로
# 키 발급을 권하기 전에 무료 경로부터 안내한다.
NO_KEY_MESSAGE = (
    "이 버튼은 Claude API를 직접 부르는 자리라 API 키가 있어야 하고, 쓰면 요금이 붙습니다.\n\n"
    "요금 없이 쓰시려면 이렇게 하세요.\n"
    "  1. 클라우드 콘텐츠 콘솔(구독으로 동작)에서 제목을 넣고 원고를 만듭니다.\n"
    "  2. 그 화면의 '원고 내보내기'를 누릅니다.\n"
    "  3. 이 앱의 '원고 가져오기' 칸에 붙여넣습니다.\n"
    "     검사·썸네일·CMS 업로드까지 그대로 이어집니다.\n\n"
    "굳이 이 버튼을 쓰시려면 프로젝트 폴더의 .env 파일을 메모장으로 열어\n"
    "아래 한 줄을 채우고 저장한 뒤, 서버 창을 닫았다가 다시 켜 주세요.\n\n"
    "    ANTHROPIC_API_KEY=sk-ant-...\n\n"
    "키는 console.anthropic.com 에서 발급하며, 구독료와는 별도로 청구됩니다.\n"
